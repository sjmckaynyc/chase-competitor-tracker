...
import time  # The secret ingredient that prevents the API from crashing
...
# 1. Target Massive High-Volume Banking & Marketing Queries
search_queries = [
...
print("Opening the floodgates. Scraping everything...")

# 2. Scrape Live Articles and Extract Real Links & Dates
for query in search_queries:
    encoded_query = urllib.parse.quote(query)
    # Hitting open Google News RSS feeds bypasses restricted paywalls and limits
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            for item in root.findall('.//item'):
                ...
                if title and link and link not in seen_links:
                    ...
                    # Extract the exact, real-world publication date
                    try:
                    ...
                    all_raw_articles.append({
                        "title": title,
                        "link": link, # Captures the verified, functional article URL
...
print(f"Found {len(all_raw_articles)} live articles. Processing ALL of them through the AI Filter...")

processed_data = []
today_stamp = datetime.today().strftime('%b %d, %Y')

# 4. Batch Process through the AI to Format and Strictly Filter
batch_size = 10
...
DO NOT return every article. Act as a strict intelligence filter. You must DISCARD any article that is:
...
ONLY return objects for articles that represent a clear strategic move, new ad campaign, credit card launch, rewards change, or direct competitive threat against Chase's LOBs.
...
"""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=45) as response:
            ...
            # Clean formatting to ensure JSON saves correctly
